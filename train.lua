...
--   self.optimState.learningRate = self:learningRate(epoch)
   ...
   for n, sample in dataloader:run() do
      ...
      self.optimState.learningRate = self:learningRate_linear(epoch, n, trainSize)
      if n==1 then print('lr = '..self.optimState.learningRate) end
...
function Trainer:learningRate_linear(epoch, n, batches_per_epoch)
   local total_epochs = self.opt.nEpochs
   local total_steps = batches_per_epoch*total_epochs
   local cur_step = (epoch-1)*batches_per_epoch+(n-1)
   return self.opt.LR*(1.0 - cur_step/total_steps)
end
